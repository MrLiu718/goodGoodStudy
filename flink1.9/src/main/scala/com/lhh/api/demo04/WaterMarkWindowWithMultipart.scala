...
object WaterMarkWindowWithMultipart {
  def main(args: Array[String]): Unit = {
    ...
    val result: DataStream[String] = sourceStream.map(x => (x.split(",")(0), x.split(",")(1).toLong))
      .assignTimestampsAndWatermarks(new MyOwnWaterMark3)
      .keyBy(0)
      .timeWindow(Time.seconds(3))
      // .allowedLateness(Time.seconds(2)) // 允许事件迟到 2 秒
      .sideOutputLateData(outputTag)
      .process(new MySumFunction3)
...
class MySumFunction3 extends ProcessWindowFunction[(String,Long),String,Tuple,TimeWindow]{
...
class MyOwnWaterMark3 extends AssignerWithPeriodicWatermarks[(String,Long)]{
  ...
  override def extractTimestamp(element: (String, Long), previousElementTimestamp: Long): Long = {
    ...
    var log:String =  "当前线程ID："+id+"event数据为" + element + "|" + "数据event_time为："+format.format(element._2)  + "|" + "当前数据最大event_time为："+format.format(currentMaxEventTime)   + "|" + "当前watermark值为："+ format.format(getCurrentWatermark().getTimestamp())
    println(log)
