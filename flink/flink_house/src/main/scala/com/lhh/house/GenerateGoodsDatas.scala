...
object GenerateGoodsDatas {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //读取csv文件
    val fileSource: DataSet[String] = environment.readTextFile("file:///F:\\开课吧 大数据\\flink\\Flink实时数仓课件\\数据\\实时数仓建表以及数据\\kaikeba_goods.csv")
    ...
    productSet.output(JDBCOutputFormat.buildJDBCOutputFormat()
      .setBatchInterval(2)
      .setDBUrl("jdbc:mysql://hadoop120:3306/flinkTest?characterEncoding=utf-8")
